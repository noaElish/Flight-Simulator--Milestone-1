...
void putInSymbolMap(string str) {
  cout<<"in put in symbol map - str:  "<<str<<endl;
  SymbolTable *instance = SymbolTable::getInstance();
  string sim = instance->getVariable(str).getSim();
  float newValue = 0;
  newValue = instance->getValueFromSim(sim);
  string dir = instance->symbolTable.find(str)->second.getDir();
  instance->updateValImSymbolTable(str, newValue);
...
double Inter::intFromString(string strVariable){
  ...
  if (symbolTableMap->symbolTable.count(str)>0)
  {
    ...
    cout<<"ex1 - vaiable not in symbol table. str: "<<str<<endl;
    unsigned count=0;
